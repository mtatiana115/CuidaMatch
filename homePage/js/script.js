...
async function validarCuidador() {
	try {
        const response = await fetch(`${URLcuidador}?email=${emailCg.value}&password=${passwordCg.value}`);
...
async function validarBeneficiario() {
	try {
        const response = await fetch(`${URLbeneficiarios}?email=${emailCb.value}&password=${passwordCb.value}`);
        const data = await response.json();
        return data.length;
    } catch (error) {
        return false;
